...
#define Mx 100007
string st,ss;
int test,num,val1,val2,len;
int tree[4*Mx],lazy[3*Mx];

bool noOver(int b, int e, int qb, int qe){
   if(b>qe||e<qb)return false;
   else return true;
}
...
void Update(int node, int b, int e, int qb, int qe){
    int left=node*2;
    int right=left+1;
   int mid =(b+e)/2;
/*
...
/*
  ...
  Update(left,b,mid,qb,qe);
  Update(right,mid+1,e,qb,qe);
 // tree[node]=tree[left]+tree[right];

}

int query(int node, int b, int e,int qb){
int mid=(b+e)/2;
int left=node*2;
int right=left+1;
/*
...
*/
  if(!noOver(b,e,qb,qb))return 0;

if(b==qb && e==qb)return tree[node];

if(qb<=mid)return(query(left,b,mid,qb)^tree[node]);
else return query(right,mid+1,e,qb)^tree[node];
}
...
while(test--){
  // memset(lazy,0,sizeof(lazy));
    ...
    if(ss=="I"){
        cin>>val1>>val2;
        Update(1,0,len-1,val1-1,val2-1);
    }
    else{
        cin>>val1;
        int v=query(1,0,len-1,val1-1);
        cout<<v<<endl;
    }
    }
}
